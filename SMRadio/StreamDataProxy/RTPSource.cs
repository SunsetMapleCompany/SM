    ...
    public class RtpSource
    {
        ...
        private void HandleIncomingPacket(byte[] packetBuffer, int packetSize)
        {
            ...
            try
            {
                ...
                position = Utility.BytesToLong(bytesPosition);
                ...
                frameSize = Utility.BytesToInteger(bytesFrameSize);
            ...
            {
                Utility.OutputMessage(false, MsgLevel.Error, "RtpSource -- HandleIncomingPacket", e.Message);
            }
