        ...
        public override void SendDatagram(byte[] sendBuffer, int sendBufferSize)
        {
            lock (mutex)
            {
                ...
                catch (System.Exception e)
                {
                    ...
                    Utility.OutputMessage(false, MsgLevel.Error, "ClientSocketTcp -- SendDatagram", e.Message);
                }
            }
        }
        ...
        public override void ReceiveDatagram()
        {
            lock (mutex)
            {
                ...
                catch (System.Exception e)
                {
                    this.OnClientTeardown();
                    this.OnExceptionOccurred(e);
                    Utility.OutputMessage(false, MsgLevel.Error, "ClientSocketTcp -- ReceiveDatagram", e.Message);
                }
            }
        }
        ...
        protected override void EndSendDatagram(IAsyncResult iar)
        {
            lock (mutex)
            {
                ...
                catch (System.Exception e)
                {
                    this.OnClientTeardown();
                    this.OnExceptionOccurred(e);
                    Utility.OutputMessage(false, MsgLevel.Error, "ClientSocketTcp -- EndSendDatagram", e.Message);
                }
            }
        }
        ...
        protected override void EndReceiveDatagram(IAsyncResult iar)
        {
            lock (mutex)
            {
                ...
                catch (System.Exception e)
                {
                    this.OnClientTeardown();
                    this.OnExceptionOccurred(e);
                    Utility.OutputMessage(false, MsgLevel.Error, "ClientSocketTcp -- EndReceiveDatagram", e.Message);
                }
            }
        }
